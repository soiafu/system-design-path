...
def build_and_solve(period_demands, day_name="Saturday", pwl_points=8, verbose=True):
    ...
    demand_lt = {
        t: aggregate_od_to_line_demands(period_demands[t], od_to_line)
    ...
    f_hi = 1.0 / H_min
    N_max_by_line = {
        L: int(math.ceil(f_hi * T_cycle[L])) for L in LINES
    }
    ...
    for L in LINES:
        for t in TIME_PERIODS:
            ...
            N[L, t] = m.addVar(
                vtype=GRB.INTEGER, lb=0, ub=N_max_by_line[L], name=f"N_{name}"
            )
            f[L, t] = m.addVar(
                vtype=GRB.CONTINUOUS, lb=f_lo, ub=f_hi, name=f"f_{name}"
            )
            E[L, t] = m.addVar(
                vtype=GRB.CONTINUOUS, lb=0.0, name=f"E_{name}"
            )
            W[L, t] = m.addVar(
            ...
            for k in allowed_k:
                z[L, t, k] = m.addVar(
                    vtype=GRB.BINARY, name=f"z_{name}_{k}"
                )
                Y[L, t, k] = m.addVar(
                    vtype=GRB.CONTINUOUS,
                    lb=0,
    ...
    for L in LINES:
        for t in TIME_PERIODS:
            ...
            # f = N / cycle time
            m.addConstr(
                f[L, t] == N[L, t] / T_cycle[L],
                name=f"link_f_N_{L}_{t}",
            )

            # exactly one train length
            m.addConstr(
                gp.quicksum(z[L, t, k] for k in allowed_k) == 1,
                name=f"onehot_{L}_{t}",
            )

            # linearization Y_k = N * z_k
            for k in allowed_k:
                m.addConstr(Y[L, t, k] <= M * z[L, t, k])
                m.addConstr(Y[L, t, k] <= N[L, t])
            ...
            seats_ph_expr = (
                gp.quicksum(k * Y[L, t, k] for k in allowed_k)
                * seats_coeff
            )

            # must serve all demand
            m.addConstr(
                seats_ph_expr * D_ur_t[t] >= dem,
                name=f"cap_{L}_{t}",
            )

            # empty seats
            m.addConstr(
                E[L, t] >= seats_ph_expr - demand_ph,
                name=f"empty_{L}_{t}",
            )
            ...
            # waiting time PWL: W = 1/f
            xs = np.linspace(f_lo, f_hi, pwl_points)
            ys = [1.0 / x for x in xs]
            m.addGenConstrPWL(
                f[L, t], W[L, t], xs.tolist(), ys, name=f"pwl_{L}_{t}"
            )
