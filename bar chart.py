...
# -------------------------------
INPUT_FILE = "./output/path_utilization_comparison.csv"  # Input file path
OUTPUT_DIR = "figure"                           # Output directory for images
...
# -------------------------------
# Clean column names: Remove special characters, keep underscores
df.columns = df.columns.str.replace(r'[^a-zA-Z0-9_]', '', regex=True)

# Map key column names (Ensure these match your CSV columns)
# Expected columns in CSV: Bench_Util_Pct, Opt_Util_Pct, Period, Line, Day
...
# Force numeric conversion for utilization columns
...
# Create figure: rows = number of lines, columns = number of days
...
bar_width = 0.35
# Set Y-axis limit slightly higher than max value for labels
y_max = df[[col_benchmark, col_optimized]].max().max() * 1.15
...
for i, line in enumerate(lines):
    for j, day in enumerate(days):
        ...
        # Filter data: Specific Line + Specific Day, sorted by Period
        ...
        # --- Color Logic ---
        # If line name contains "NWK" or "WTC", use Red scheme; otherwise use Blue scheme
        ...
        # Only show Y-axis label on the first column to reduce clutter
        if j == 0:
        ...
        # --- Value Annotation (Optimized Only) ---
        for rect in rects2:
...
# 4. Layout and Saving
# -------------------------------
# Unified Legend (Take handles from the first subplot)
...
# Save Figure
save_path = os.path.join(OUTPUT_DIR, "utilization_comparison_grouped.png")
